...

{
  options = {
    users.personalUser = {
      enableSshLogin = lib.mkOption {
        ...
        default = false;
        description =
          "Enable SSH login for the personal user with authorized keys";
      };
    };
  };
  ...
  config = {
    services.openssh.enable =
      lib.mkIf config.users.personalUser.enableSshLogin true;
    ...
    users.users.griff = {
      ...
      openssh.authorizedKeys.keys =
        lib.mkIf config.users.personalUser.enableSshLogin [
  ...
  services.onepassword.polkitPolicyOwners = [ "griff" ];
}
