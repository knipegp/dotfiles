...
Personal system configuration using NixOS flakes for declarative and reproducible system management across Linux and macOS.
...
- **Triple flake structure**: Separate flakes for system, user, and Darwin configurations
- **Remote deployment**: Deploy configurations to other machines via SSH
- **Modern development environment**: Comprehensive tooling for development work
- **Window managers**: Hyprland and Sway with shared configurations
- **Reproducible builds**: Flake lockfile ensures consistent environments
...
3. Create ``nixos/hosts/<hostname>/configuration.nix`` importing desired modules
...
1. Edit ``nixos/darwin/flake.nix`` and update:

   - ``hostname`` variable
   - ``system`` variable (``aarch64-darwin`` or ``x86_64-darwin``)

2. Customize ``nixos/darwin/configuration.nix`` as needed
...
- **Modern CLI tools**: ``eza``, ``ripgrep``, ``bat``, ``fd``, ``bottom``
- **Development environments**: Python, Nix, shell scripting
- **Git configuration**: GPG signing, difftastic, rebase workflow
- **Editor support**: Doom Emacs configuration (manual install required)
- **Pre-commit hooks**: ``prek`` for automated quality checks

Pre-commit Hooks
----------------

This repository uses ``prek`` for pre-commit hooks. Always run checks before committing:
...
1. **NixOS System** (``nixos/flake.nix``)
